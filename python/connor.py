...
while True:
  ...
  if today != prevday:
    ...
    x = datetime.datetime.now()
    day = x.strftime("%A")
    if os.path.isfile('temp/tempc'):
        os.remove('temp/tempc.txt')
    with open('temp/tempc.txt', 'w') as t:
        t.close()
    os.system('cls')
    
    t = open('temp/tempc.txt', 'a')
    t.write("The date today is:", day, today)
    t.write("\n")
